...
class CustomSeekbarLoading(context: Context) : View(context) {
    ...
    init {
        ...
        paint_Text = Paint(Paint.ANTI_ALIAS_FLAG).apply {
            ...
            typeface = try {
                Utils.getTypeFace("display", "display_bold.otf", context)
            } catch (e: Exception) {
                Utils.getTypeFace("display", "display_bold.ttf", context)
            }
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        if (progress > 0) rectF2[padding * 2.2f, padding * 2.2f, width - progress * rectF.width() / 100] =
            height - padding * 2.2f
        else rectF2[padding * 2.2f, padding * 2.2f, width - progress * rectF.width() / 100 - padding * 2.2f] =
            height - padding * 2.2f
        path.reset()
        path.addRoundRect(rectF2, floatsRadius2, Path.Direction.CW)
        canvas.drawPath(path, paintPr)
        drawCenter(canvas, paint_Text, "${102 - progress} %")
    }

    fun setProgress(progress: Int) {
        this.progress = 102 - progress
        invalidate()
    }
